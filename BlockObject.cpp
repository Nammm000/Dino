...
bool Block::InitBlock(SDL_Renderer* screen , const int& xp)
{
    int number = SDLCommonFunc::MyRandom(1, 10);
    int number2 = number;
    number = number * 14;

    bool ret1 = m_Topblock.LoadImg("img//topblock22.png", screen);
    bool ret2 = m_BottomBlock.LoadImg("img//bottomblock22.png", screen);
...
void Block::RenderImg(SDL_Renderer* screen)
{
...
SDL_Rect Block::GetTopRect()
{
   return m_Topblock.GetRectPos();
}

void Block::SetRectVal(const UINT& xp)
{
    m_Topblock.SetXPos(xp);
    m_BottomBlock.SetXPos(xp);
}

void Block::SetIsBack(bool isBack)
{
    is_back_ = isBack;
...
void Block::DrawBound(SDL_Renderer* des)
{
...
bool Block::CheckPass(SDL_Rect rect)
{
    bool ret = SDLCommonFunc::CheckCollision(rect, slot_rect_);
    return ret;
}

bool Block::CheckCol(SDL_Rect pl_rect)
{
...
{
    int n = m_BlockList.size();
    for (int i = 0; i < n; i++) {
        Block* pBlock = m_BlockList.at(i);
        delete pBlock;
        pBlock = NULL;
    }
...
{
    Block* m_block1 = new Block();
    Block* m_block2 = new Block();
    Block* m_block3 = new Block();
    Block* m_block4 = new Block();
    Block* m_block5 = new Block();
    Block* m_block6 = new Block();
...
void BlockManager::Render(SDL_Renderer* screen) {
    ...
    for (int i = 0; i < n; i++) {
        ...
        if (!stop_moving_) {
            ...
            if (ret == true) {
                Block* endBlock = m_BlockList.at(xp_max_);
