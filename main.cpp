...
bool InitData()
{
    ...
    g_window = SDL_CreateWindow("Dino",
...
bool LoadBackground()
{
    ...
    return g_background.LoadImageFile("img//background000.png", g_screen);
}
...
int main(int argc, char* argv[])
{
    ...
    //Make menu game
    int ret_menu = SDLCommonFunc::ShowMenu(g_screen, g_font_MENU, "Play Game", "Exit", "img//background000.png");
    if (ret_menu == 1) {
            quit = true;
    }
    ...
    bool ret = g_ground.LoadImageFile("img//ground2222.png", g_screen);
    if (!g_ground.LoadImageFile("img//ground2222.png", g_screen)) {
        return -1;
    }
    g_ground.SetRect(0, GROUND_MAP);
    again_label:

    TextObject text_count;//_
    text_count.setColor(TextObject::WHITE_TEXT);

    PlayerObject player;

    player.SetRect(100, 100);
    if (!player.LoadImg("img//gwedly1.png", g_screen)) {
        return -1;
    }

    BlockManager manage_block;

    if (!manage_block.InitBlockList(g_screen))
        return -1;

    while (!quit) {
        fps.start();
        while (SDL_PollEvent(&g_event) != 0) {
            Mix_Music* music = Mix_LoadMUS("sound//Free Game Loop2.wav");
            if (music != NULL)
        ...
        if (game_over == true) {
            Sleep(500);
            int ret_menu = SDLCommonFunc::ShowMenuEnd(g_screen, g_font_MENU,
                                                   "Player Again", "Exit",
                                                    "img//ground2222.png");
            if (ret_menu == 1) {
                quit = true;
        ...
        if (val1 < 1000 / FRAMES_PER_SECOND) {
            SDL_Delay((1000 / FRAMES_PER_SECOND) - val1);
        }
    }
